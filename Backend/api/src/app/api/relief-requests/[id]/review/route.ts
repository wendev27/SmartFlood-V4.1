...
import { ReliefRequestError, reviewReliefRequest, type ReliefRequestAction } from "@/lib/reliefRequests";
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (action !== "approve" && action !== "reject") return NextResponse.json({ success: false, error: "action must be approve or reject." }, { status: 400 });
    const { id } = await context.params;
    return NextResponse.json({ success: true, data: await reviewReliefRequest(id, viewer, action as ReliefRequestAction, body) });
  } catch (error) {
